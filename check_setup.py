...
Checks your Python version, the installed packages, your chosen PROVIDER, and (if
you switched off the offline mock) the API key that provider needs. Makes NO API
calls. Uses only the standard library, so it runs even before `pip install`.

The default PROVIDER=mock needs no key and runs the entire repo offline.
"""
...
ALWAYS = [
    ...
    ("rich", "rich", "the event trace and capstone output"),
]
PROVIDER_DEPS = {
    "mock": [],  # the whole point: no SDK needed to run the repo
...
def check_provider(env):
    ...
    if provider in PROVIDER_DEPS:
        ok(f"PROVIDER = {provider}" + ("  (offline, no key)" if provider == "mock" else ""))
        return provider
...
def main():
    print(_c("Checking your setup for the Agent Harness deep dive...\n", "1"))
    env = _read_env_file()
    py = check_python()
